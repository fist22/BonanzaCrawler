...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["consumer_electronics"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["consumer_electronics"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["consumer_electronics"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["consumer_electronics"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["consumer_electronics"]["page"],'a')

    print 'Consumer_Electronics Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[10]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[10].find('a').text
    ...
    end_time = time.time()
    print 'Consumer_Electronics End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Consumer_Electronics Product URLs collected in : ' + str(int(end_time - start_time)) + 's'