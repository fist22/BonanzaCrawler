...
# file = open(constants.file_names["dvds_movies"]["product"], 'a')
...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["everything_else"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["dvds_movies"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["dvds_movies"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["everything_else"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["everything_else"]["page"],'a')

    print 'Everything_Else Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[16]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[16].find('a').text
    ...
    end_time = time.time()
    print 'Everything_Else End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Everything_Else Product URLs collected in : ' + str(int(end_time - start_time)) + 's'