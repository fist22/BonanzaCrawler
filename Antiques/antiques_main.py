...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["antiques"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["antiques"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["antiques"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["antiques"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["antiques"]["page"],'a')

    print 'Antiques Start Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[0]
    ...
    end_time = time.time()
    print 'Antiques End Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    print 'Antiques Product URLs collected in : ' + str(int(end_time - start_time)) + 's'