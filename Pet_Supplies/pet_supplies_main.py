...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["pet_supplies"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["pet_supplies"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["pet_supplies"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["pet_supplies"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["pet_supplies"]["page"],'a')

    print 'Pet_Supplies Start Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[24]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[24].find('a').text
    ...
    end_time = time.time()
    print 'Pet_Supplies End Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    print 'Pet_Supplies Product URLs collected in : ' + str(int(end_time - start_time)) + 's'