...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["home_garden"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["home_garden"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["home_garden"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["home_garden"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["home_garden"]["page"],'a')

    print 'Home_Garden Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[19]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[19].find('a').text
    ...
    end_time = time.time()
    print 'Home_Garden End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Home_Garden Product URLs collected in : ' + str(int(end_time - start_time)) + 's'