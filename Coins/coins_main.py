...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["coins"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["coins"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["coins"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["coins"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["coins"]["page"],'a')

    print 'Coins Start Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[7]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[7].find('a').text
    ...
    end_time = time.time()
    print 'Coins End Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    print 'Coins Product URLs collected in : ' + str(int(end_time - start_time)) + 's'