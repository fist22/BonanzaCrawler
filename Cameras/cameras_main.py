...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["cameras"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["cameras"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["cameras"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["cameras"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["cameras"]["page"],'a')

    print 'Cameras Start Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[5]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[5].find('a').text
    ...
    end_time = time.time()
    print 'Cameras End Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    print 'Cameras Product URLs collected in : ' + str(int(end_time - start_time)) + 's'