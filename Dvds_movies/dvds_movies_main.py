...
def sub_cat_collect():
    ...
    success_urls_file = open(constants.file_names["dvds_movies"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["dvds_movies"]["page"],'a')

    print 'Dvds_Movies Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[14]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[0].find('a').text
    ...
    end_time = time.time()
    print 'Dvds_Movies End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Dvds_Movies Product URLs collected in : ' + str(int(end_time - start_time)) + 's'