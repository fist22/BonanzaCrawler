...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["digital_goods"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["digital_goods"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["digital_goods"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["digital_goods"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["digital_goods"]["page"],'a')

    print 'Digital_Goods Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[12]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[12].find('a').text
    ...
    end_time = time.time()
    print 'Digital_Goods End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Digital_Goods Product URLs collected in : ' + str(int(end_time - start_time)) + 's'