...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["crafts"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["crafts"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["crafts"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["crafts"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["crafts"]["page"],'a')

    print 'Crafts Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[11]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[11].find('a').text
    ...
    end_time = time.time()
    print 'Crafts End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Crafts Product URLs collected in : ' + str(int(end_time - start_time)) + 's'