...
# file = open(constants.file_names["entertainment"]["product"], 'a')
...
def sub_cat_collect():
    ...
    try:
        ...
        success_cat_urls= open(constants.file_names["entertainment"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["entertainment"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["entertainment"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["entertainment"]["page"],'a')

    print 'Entertainment Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[15]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[15].find('a').text.replace(' ','_').replace(',','').encode('utf8','ignore')
    print main_category
    ...
    end_time = time.time()
    print 'Entertainment End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Entertainment Product URLs collected in : ' + str(int(end_time - start_time)) + 's'