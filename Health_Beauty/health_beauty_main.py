...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["health_beauty"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["health_beauty"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["health_beauty"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["health_beauty"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["health_beauty"]["page"],'a')

    print 'Health_Beauty Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[18]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[18].find('a').text
    ...
    end_time = time.time()
    print 'Health_Beauty End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Health_Beauty Product URLs collected in : ' + str(int(end_time - start_time)) + 's'