...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["stamps"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["stamps"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["stamps"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["stamps"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["stamps"]["page"],'a')

    print 'Stamps Start Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[29]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[29].find('a').text
    ...
    end_time = time.time()
    print 'Stamps End Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    print 'Stamps Product URLs collected in : ' + str(int(end_time - start_time)) + 's'