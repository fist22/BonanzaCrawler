...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["video_games"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["video_games"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["video_games"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["video_games"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["video_games"]["page"],'a')

    print 'Video_Games Start Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[33]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[33].find('a').text
    ...
    end_time = time.time()
    print 'Video_Games End Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    print 'Video_Games Product URLs collected in : ' + str(int(end_time - start_time)) + 's'