...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["music"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["music"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["music"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["music"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["music"]["page"],'a')

    print 'Music Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[21]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[21].find('a').text
    ...
    end_time = time.time()
    print 'Music End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Music Product URLs collected in : ' + str(int(end_time - start_time)) + 's'