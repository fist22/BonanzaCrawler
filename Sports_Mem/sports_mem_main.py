...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["sports_mem"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["sports_mem"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["sports_mem"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["sports_mem"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["sports_mem"]["page"],'a')

    print 'Sports_Mem Start Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[28]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[28].find('a').text
    ...
    end_time = time.time()
    print 'Sports_Mem End Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    print 'Sports_Mem Product URLs collected in : ' + str(int(end_time - start_time)) + 's'