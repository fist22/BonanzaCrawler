...
def get_prod_url():
    for sub_cat_url in sub_cat_urls:
        ...
        if url not in sub_cat_urls:
            ...
            tot_pages = int(
                bsoup.find_all('div', {'class': 'scroll_progress_bar_container'})[0].get('title').split(' ')[-1])
...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["business"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls = open(constants.file_names["business"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["business"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["business"]["success"], 'a')

    global category_pages_file
    category_pages_file = open(constants.file_names["business"]["page"], 'a')

    print 'Business Start Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[4]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[4].find('a').text
    for x in category.find_all('div', {'class': 'sub_category_list'})[0].find_all('a', {'class', 'link_to_search'}):
        ...
        sub_categories(x.get('href').replace('search', 'facets') + url_suffix,
                       x.text.replace('&', 'and').replace(' ', '_') + '|')
        print x.text

    get_prod_url()

    end_time = time.time()
    print 'Business End Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    print 'Business Product URLs collected in : ' + str(int(end_time - start_time)) + 's'