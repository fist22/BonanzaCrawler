...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["parts_accessories"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["parts_accessories"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["parts_accessories"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["parts_accessories"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["parts_accessories"]["page"],'a')

    print 'Parts_Accessories Start Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[23]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[23].find('a').text
    ...
    end_time = time.time()
    print 'Parts_Accessories End Time: ' + str(datetime.datetime.now().strftime("%d-%m-%y %H:%M:%S"))
    print 'Parts_Accessories Product URLs collected in : ' + str(int(end_time - start_time)) + 's'