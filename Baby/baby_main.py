...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["baby"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls = open(constants.file_names["baby"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["baby"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["baby"]["success"], 'a')

    global category_pages_file
    category_pages_file = open(constants.file_names["baby"]["page"], 'a')

    print 'Baby Start Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[2]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[2].find('a').text
    ...
    end_time = time.time()
    print 'Baby End Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    print 'Baby Product URLs collected in : ' + str(int(end_time - start_time)) + 's'