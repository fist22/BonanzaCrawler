...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["collectibles"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["collectibles"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["collectibles"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["collectibles"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["collectibles"]["page"],'a')

    print 'Collectibles Start Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[8]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[8].find('a').text
    ...
    end_time = time.time()
    print 'Collectibles End Time: ' + str(datetime.datetime.now().strftime("%d-%ym-%y %H:%M:%S"))
    print 'Collectibles Product URLs collected in : ' + str(int(end_time - start_time)) + 's'