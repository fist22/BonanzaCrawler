...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["musical_instruments"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["musical_instruments"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["musical_instruments"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["musical_instruments"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["musical_instruments"]["page"],'a')

    print 'Musical_Instruments Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[22]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[22].find('a').text
    ...
    end_time = time.time()
    print 'Musical_Instruments End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Musical_Instruments Product URLs collected in : ' + str(int(end_time - start_time)) + 's'