...
def sub_cat_collect():
    ...
    global file
    file = open(constants.file_names["fashion"]["product"], 'a')

    global sub_cat_urls
    try:

        success_cat_urls= open(constants.file_names["fashion"]["success"]).read().splitlines()
        page_urls = open(constants.file_names["fashion"]["page"]).read().splitlines()
    ...
    success_urls_file = open(constants.file_names["fashion"]["success"],'a')
    
    global category_pages_file
    category_pages_file = open(constants.file_names["fashion"]["page"],'a')

    print 'Fashion Start Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    ...
    bsoup = set_bsoup(url)
    category = bsoup.find_all('div', {'class': 'category_group_container'})[17]
    main_category = bsoup.find_all('div', {'class': 'category_group_container_mid'})[17].find('a').text
    ...
    end_time = time.time()
    print 'Fashion End Time: ' + str(datetime.datetime.now().strftime("%H:%M:%S"))
    print 'Fashion Product URLs collected in : ' + str(int(end_time - start_time)) + 's'